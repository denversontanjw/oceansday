...
import { getRandomOceanFact } from "@/lib/ocean-facts";

type ResultState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "success"; gift: string; fact: string }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.status === "success") {
        setResult({ status: "success", gift: data.gift, fact: getRandomOceanFact() });
      ...
      <div ref={resultRef} aria-live="polite" className="mt-5">
        {result.status === "success" && GiftPic && (
          <div className="animate-pop rounded-3xl bg-white p-6 text-center shadow-2xl shadow-ocean-deep/30 sm:p-8">
            ...
            <div className="mt-5 rounded-xl bg-sand/40 px-4 py-3 text-left">
              <p className="text-xs font-semibold uppercase tracking-wide text-ocean-700">🌊 Did you know?</p>
              <p className="mt-1 text-sm text-slate-700">{result.fact}</p>
            </div>
          </div>
        )}
